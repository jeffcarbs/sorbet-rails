...
class SorbetRails::ActiveRecordRbiFormatter
  ...
  sig {returns(String)}
  def generate_active_record_batches_batch_enumerator_rbi
    puts "-- Generate sigs for ActiveRecord::Batches::BatchEnumerator --"
    ...
    parlour.root.create_class('ActiveRecord::Batches::BatchEnumerator') do |class_rbi|
      class_rbi.create_include("Enumerable")
      class_rbi.create_extend("T::Generic")
      class_rbi.create_constant(
        "Elem",
        value: "type_member",
      )
      ...
      class_rbi.create_method(
        "each_record",
        parameters: [Parameter.new("&block", type: "T.nilable(T.proc.params(e: ItemElem).void)"),],
        return_type: "T::Enumerator[ItemElem]",
      )

      delegated_methods = %w(update_all delete_all destroy_all)
